...
### Q-W12-07 - Notes model: single-table unify vs keep-both-with-view - REGISTERED for owner (W12-12)

Notes are fragmented across five stores; three surfaces write three different ones and the one table linking a patient + an appointment (`appointment_notes`) has NO writer, so an Agenda note never reaches the patient profile (SPEC-notes-unification.md section 1). The build (W12-13) must pick a model.
**Recommended default (SPEC-notes-unification.md section 3):** unify onto ONE relation - make `appointment_notes.appointment_id` NULLABLE (a note has `patient_id` always + optional `appointment_id` + optional `episode_id`; `author_user_id` also nullable for backfilled system rows), backfill `appointments.notes` + `patient_note_revisions` into it (non-destructive: read old, insert new, never erase the append-only history), and read every surface from it. Fallback (only if the owner wants the two histories physically separate): keep both tables behind a read-union view. Minor sub-note: the Agenda drawer's edit-in-place "Notas" becomes append-a-revision (consistent with append-only); confirm that is acceptable vs a single mutable note per appointment. No build until this is ruled; the default stands if the owner does not object.

### Q-W12-08 - Equipa: staff-location model + per-location colour storage - REGISTERED for owner (W12-14)
...
### Q-W12-10 - Can reception create time_off blocks? (matrix/RLS decision) - REGISTERED for owner (W12-28)

W12-28 added a "Bloquear horário" affordance on the agenda to retire reception's informal "Não Marcar" fake-appointment hack. But `createTimeOffBlock` server-enforces `settings:manage` (admin/owner only per the matrix); reception has `appointments:write`, NOT `settings:manage`. So the shipped affordance is gated on `settings:manage` (admin/owner) - it NEVER relaxes the guard (loop Field 5). The loop's recommended default ("reception can create blocks in its scope") is incompatible with the current guard and would require a MATRIX + RLS change - a security decision, not guessed here.
**Recommended default (owner to rule):** to actually retire reception's hack, add a NARROW reception block capability scoped to the therapists/locations reception already serves (e.g. an `appointments:block` capability, or extend `time_off` writes for reception within its location scope), with the RLS + isolation test reviewed by CYAN. Alternative: keep blocks admin/owner-only and have reception ask an admin. Until ruled, the agenda affordance stays admin/owner (settings:manage); the model + stale-comment wiring are done, so relaxing the gate on a ruling is a one-line follow-up. Any "Não Marcar" fake-patient cleanup is a separate owner-gated data window.

### ANSWERED 2026-07-23 EVE (owner batch R4/R16) - W12-15 migration 0038 rulings

- **Q-W10-04-1 RULED (R4/R5): staff_locations junction + one-migration close, SPLIT for safety.** The owner batch confirmed the many-to-many `staff_locations` junction (not `users.location_id`) and closing the reception-location model + per-therapist RLS tighten. GREEN SPLITS the one-migration default into TWO migrations to land the clinical-data change isolated + independently CYAN-auditable, one migration in flight: **0038 = the `staff_locations` junction ONLY** (this PR - foundation, no clinical_records change); **0039 = the access-model RLS tighten** (admin location-scoping per R16 + therapist own-patients-only on `clinical_records`). 0038 nets NO widening (net-new table); 0039 is derived-then-tightened, reception clinical read stays DENIED. See DECISIONS 2026-07-23 (W12-15 split).
- **Q-W12-08 RULED (R4): per-location colour stored on `staff_locations`.** A nullable `color text` column ships on the junction in 0038; duplicates across locations accepted (Rodica); NULL falls back to the FNV-1a hash (`therapist-color.ts`). Colour is decoupled from the RLS surface (a colour edit is a plain UPDATE under the owner/admin write policy). Feeds W12-21.
- **R16 folded into 0039 (not 0038): admins are LOCATION-SCOPED via `staff_locations`; owner = tenant-wide.** This supersedes the W10-03 "admin tenant-wide" matrix line. The RLS + server-check rewrite that derives admin scope from `staff_locations` lands in 0039 alongside the therapist tighten (both are clinical/access-control changes reading the junction), keeping 0038 a clean foundation. 0038 keeps the tenant-wide owner/admin WRITE baseline on the junction itself (the current admin power level - nothing widened); 0039 tightens admin write scope to the admin's own location(s). The 0039 isolation-test matrix is updated per R16: owner tenant-wide; admin location-scoped; therapist own-patients-only; reception clinical DENIED. CYAN attention flagged on 0039.
- **Q-W7-01-2 (orphaned-auth adopt-on-re-invite, R6)** remains a NON-migration follower of W12-15 (invite/provision path), unaffected by the 0038/0039 split; built after the junction lands.

### ANSWERED 2026-07-23 LATE - Q-W12-09 (R12) + 0039 design questions opened

- **Q-W12-09 RULED (R12): ficha v5 order.** Apply Rodica's 0-15 sequence; append the 3 unlisted v4 fields (episode_date, red_flags, bodychart) AFTER position 15 in their current relative order (removable via a future template version; no delete UI now); remove Codigos CID. Built as `osteopathy-v5.json` (W12-22, display-order only, no DDL); position 0 "Paciente dashboard" = the app patient header (not a template field); item 10 "Outros" = health_problems (the Problemas de Saude grid + Outros free-text). See DECISIONS 2026-07-23 LATE (W12-22).
- **OPEN for 0039 (routed to CYAN, escalations/W12-15-0039-clinical-rls-design-for-CYAN):** (1) the ADMIN patient->location basis R16 did not specify (recommended: via the patient's appointment locations; clinical_records has no location_id, appointment_id is nullable); (2) whether 0039 removes admin from the clinical_records WRITE policies to match the "admin read-only" matrix (recommended: yes); (3) confirmation that re-basing the existing clinical-RLS security tests to the new access model is in-scope for the 0039 PR. GREEN will not build 0039 until (1)-(3) are confirmed - no half-tightened clinical RLS lands.

### Q-W12-29-1 - slot granularity: does R13 "hour-only for both" change BOOKING or just the agenda VIEW? (portal-safety) - REGISTERED for owner/CYAN (W12-29)

W12-29 shipped migration 0041 (`locations.slot_granularity_min` smallint NOT NULL DEFAULT 30) + parameterised the PORTAL booking generator (`listOpenSlots`) to step by the location's granularity (coalesce -> 30, so default = UNCHANGED, portal-safe; a DB test proves 30 -> 30-min starts, 60 -> hourly). What is NOT built yet, pending this ruling:
1. **View vs booking coupling (portal-safety).** The loop couples one column to BOTH the agenda VIEW row-step AND booking-slot granularity. R13 says "BOTH locations hour-only VIEW ... initial value = hour-only ON for both". If R13 means only the agenda VIEW (display) should be hourly, but LV booking should stay 30, the column cannot serve both (setting LV to 60 makes LV PORTAL booking 60 too). **Confirm:** does setting a location to 60 change (a) the agenda view only [needs a SEPARATE view-granularity column], or (b) both view AND booking [the current single-column design - LV portal booking becomes 60]? Recommended default: (b) single column drives both (simpler; Q-W9-00-3 originally scoped booking granularity), and set CB=60 but LEAVE LV=30 (not "both"), i.e. reconcile R13's "both" with Q-W9-00-3's "LV unchanged".
2. **The agenda-VIEW wiring** (daySlots/agenda-grid row-step + minToPx/SLOT_HEIGHT) is NOT parameterised yet (invasive client threading) - build after (1) is settled.
3. **The admin per-location toggle** (R13 "toggle built as config") - build with (2).
4. **Setting the values** (CB and/or LV -> 60) is an owner-gated prod DATA write - not done.
GREEN built only the safe, tested foundation (column + booking-step param, default 30 = no change) and halts the view/toggle/value + the R13-vs-Q-W9-00-3 reconciliation to the owner/CYAN.

### Q-W12-21-1 - approved swatch shows green/blue/teal/purple at hexes the reused repo tokens do not match - REGISTERED for owner (W12-21 visual gate, GREEN-defaultable)

The owner-approved W12-21 swatch (2026-07-25) proposes 19 colour assignments but reuses 4 EXISTING tokens for green/blue/teal/purple rather than adding new ones: green -> `v2-green-700`, blue -> `v2-blue-700`, teal -> `accent-2-700`, purple -> `accent-1-700`. The repo's hexes for those tokens (`#4E7D6B` / `#345C9C` / `#2F7E72` / `#8B1863`) differ from the vivid hexes the swatch preview rendered (`#2E7D32` / `#1565C0` / `#00796B` / `#6A1B9A`). The token-definition loop added ONLY the 15 genuinely-new hues and did NOT redefine the 4 reused tokens, because (a) `accent-1-700` is the canonical brand magenta (`#8B1863`), locked by `tokens.test.ts` and brand-tokens.md, and (b) re-hexing `v2-green-700`/`v2-blue-700`/`accent-2-700` would drift the whole product palette (KPIs, categories, the existing hash palette) and break `tokens-v2.test.ts`. So on the agenda those 4 therapists will render the repo hexes, not the preview hexes.
**Recommended default (GREEN):** accept the reused tokens as-is (they are AA-pass and already the product's green/blue/teal/purple) - the colours are still distinct because the NAME is authoritative and the legend labels each. **If the owner wants the vivid preview hues instead:** add 4 more dedicated NEW tokens (e.g. `v2-emerald-700 #2E7D32`, `v2-azure-700 #1565C0`, a distinct teal, a distinct purple) and map green/blue/teal/purple to those - a small additive token follow-up, never a redefinition of the brand/v2 tokens. Not blocking; resolvable at the visual gate.

### Q-W12-21-2 - legend footer + agenda hash->stored-colour swap + colour seed DEFERRED to the data step (W12-40) - REGISTERED for owner (W12-21)

The W12-21 loop doc also calls for an always-visible per-location legend footer and swapping the agenda name colour from the FNV-1a hash to the stored per-(therapist, location) colour. Those are intrinsically coupled to the per-(therapist, location) colour VALUES, which are empty on `staff_locations.color` until W12-40 seeds Rodica's CB + LVA list. Building the legend / the consumption swap now would render every therapist at the hash FALLBACK (not the approved palette), so the owner visual gate could not confirm the approved colours. **Recommended default:** land the token DEFINITIONS now (this PR, the atomic owner-approved unit); the agenda consumption + the legend footer ship WITH the W12-40 seed (or a dedicated consumption follow-up right after it), so the first visible render already shows the seeded colours. No product decision pending - this is a sequencing note; confirm if the owner instead wants the legend shipped early against fallback colours.

### Q-W12-13-1 - Agenda/composer note "clear" under append-only + backfill-loop legacy-read retirement - REGISTERED for owner (W12-13, resolves the Q-W12-07 sub-note)

W12-13 (non-migration surfaces) re-pointed every note WRITE to the unified `appointment_notes` (append-only) and every READ to a transitional, dedup-safe union with the legacy stores (no feature flag, no backfill dependency - see DECISIONS 2026-07-24 W12-13). Two owner-visible consequences follow, each shipped on a recommended default:
1. **Clearing a note is now a no-op (the Q-W12-07 sub-note).** The Agenda drawer / profile composer "Notas" is append-only: an edit APPENDS a new revision and blanking the field does NOT erase the prior note (the latest non-empty note keeps showing), consistent with `patient_note_revisions`. **Recommended default:** keep append-only (matches the ruled single-table-unify model + the append-only invariant). If the owner instead wants a visible "clear/remove note", that needs a tombstone/soft-delete revision (append a delete marker the reads honour) - a separate, additive change, NOT an in-place mutate/hard delete of history.
2. **Legacy-read retirement is OPTIONAL, not required.** The profile read de-duplicates legacy `patient_note_revisions` rows against backfilled `appointment_notes` rows by the natural key (content + `created_at`), so the union is correct both PRE- and POST-backfill with no double-count. **Recommended default:** the future owner-gated BACKFILL loop MAY drop the legacy read legs (the `patient_note_revisions` union in `listPatientNotes` + the `appointments.notes` fallback in the agenda `COALESCE`) once the backfill is verified, purely as cleanup; nothing breaks if they are left in place. No action needed from this loop.

## Pre-Launch batch (registered 2026-07-27, YELLOW) - Rodica batch + Claude-found

> Batched per rule 13. Send Q-PL-04-1 and Q-PL-05-1/Q-PL-01-1 to Rodica in one
> message; Q-INC-02-1 is owner infra. Loops: `docs/loops/prelaunch/`.

### Q-PL-04-1 - NESA "missing": already a seeded service - WHERE is it missing? - OWNER-CONFIRMED PRESENT 2026-07-28; awaiting Rodica's specific context (PL-04)

**OWNER NOTE 2026-07-28:** "I also see it is present." NESA the service exists and is visible, corroborating the recon (seeded in the catalogue). No fix to build. Only open thread: if Rodica still sees it "missing" it is a specific context (a booking flow, or a therapist without the NESA therapist_services mapping - e.g. David Batista, see the roster correction), not the catalogue. Downgraded from HALTED to a Rodica-context follow-up.

**FULLY DIAGNOSED 2026-07-28 (attested read-only prod, sha256 52781a27):** NESA service EXISTS (id 270fb115, active) AND David Batista (the NESA therapist) IS mapped to it, at Linda-a-Velha - so no missing service and no mapping gap. David is LV-ONLY; Tiago Grilo is CB but maps Fisioterapia, not NESA. So NESA will not appear when booking at **Castelo Branco** or via a non-NESA therapist - the likely "Falta NESA". Resolution is a ROSTER decision (assign NESA to a CB therapist if CB needs it, or confirm NESA is LV-only), owner/Rodica, NOT a code fix. (The script's generic "data gap" flag on Tiago is a false alarm - he is a physio; and the `ilike '%nesa%'` also false-matched "Medicina Chinesa/Acupuntura", which is not a NESA service.)


Rodica: "Falta a opcao NESA" (screenshot: Terapeuta dropdown open, service selector off-screen). **Recon overturns the briefing's premise that NESA must be ADDED to the catalogue: NESA is ALREADY seeded as a service** - `wave08-catalog.ts` has "Tratamento NESA" (LAV `:59`) + "NESA" (CB `:75`) + "Pacote 10 - NESA" (`:81`), plus `form-templates/nesa-v1.json`; the full catalogue is 22 services (not 20). So a catalogue insert would be WRONG (duplicate risk). The report is ambiguous about WHERE NESA is missing. Three shapes:
- **A (recommended default) - missing for a THERAPIST.** The NESA therapist (plausibly David Batista) has no `therapist_services` mapping for NESA - or, per handoff C-1, no `users` row at all yet (David Batista is in the deferred-invite set). This fits the Terapeuta-dropdown-open screenshot and ties to G6 (invites), NOT a catalogue insert.
- **B - missing at a LOCATION.** LAV shows "Tratamento NESA", CB shows "NESA"; a location mismatch/filter could hide the expected one.
- **C - not seeded on PROD.** The seed is code; prod is real-data-only and the wave08 catalogue may not have been inserted. **CYAN read-only check (ref-guard `dfotoodqvmjhbdcxyaxf`):** confirm the NESA services exist on prod, which therapists carry a NESA `therapist_services` mapping, and whether David Batista has a `users` row. This decides A vs C before any fix.

**Ask Rodica:** when NESA is "missing", is it (A) for a specific therapist, (B) at a specific clinic, or (C) not there at all? If C, the fix is a prod-data seed under an AUTORIZO phrase, not a catalogue edit. YELLOW authored the question only; no fix built.

### Q-PL-05-1 - Terapeuta dropdown "bookable" definition must not drop the practicing owner (JP) - REGISTERED for owner (PL-05, GREEN-defaultable)

The Terapeuta dropdown reads all staff (`data.ts:242-248` excludes only `reception`), so "Ivan M" (owner) and "Lurdes Cruz" (admin) are selectable. A naive `role='therapist'` filter would WRONGLY drop the owner JP, who is a practicing osteopath (role=owner). The distinguishing signal is `therapist_services` (practitioners carry service mappings; Ivan M + Lurdes do not). **Recommended default (GREEN):** source the dropdown from bookable practitioners = `role='therapist'` OR has >= 1 `therapist_services` mapping, keeping JP and dropping Ivan M + Lurdes + reception. Confirm this "bookable" definition, or state the exact roster of who should be bookable. Needs the CYAN role/mapping inventory (also feeds handoff C-1) to be sure no real therapist is dropped.

### Q-PL-01-1 - Agenda same-hour: vertical stack is intentional + migration 0041 is inert - confirm the fix direction - ANSWERED 2026-07-28 by merge (scroll-within-band accepted) (PL-01, CLOSED)

**ANSWERED 2026-07-28:** owner merged #675, which ships the recommended default - keep the vertical stack, bound each same-hour group to its hour band, overflow scrolls WITHIN (no columns, 0041 not activated, names not truncated). Bound mechanism = scroll accepted; owner may still ask for "+N mais" or compact lines as a later tweak. Not blocking.


Rodica: "as marcacoes descem para a hora seguinte" (4 appts at 09:00 stack downward past their hour). **Briefing-versus-reality mismatch (rule 11):** the briefing hypothesised "split into columns" on an "hour-only grid (migration 0041)". Recon found (a) the vertical stack is INTENTIONAL and locked by a test (`agenda-grid.tsx:39-46`; `agenda-cards.spec.ts:104` asserts same-time appts stack vertically, equal x different y), and (b) `slot_granularity_min` (0041, CB->60) is STORED but has ZERO grid consumers, so there is no hour-only grid in effect. **Recommended default:** keep the vertical stack and BOUND it to the hour band so all same-hour rows read within their hour (compact/clamp/"+N"), which satisfies the DoD ("all four inside the 09:00 band") WITHOUT columns and WITHOUT activating 0041. If Rodica actually wants side-by-side columns, that is a design change that regresses the tested contract - confirm before building.

### Q-INC-02-1 - Provision a named non-prod test target for Rodica (incident root cause) - REGISTERED for owner (INC-02 half a, INFRA)

Rodica keeps testing on prod (synthetic "Teste CB" + appts + a Declaracao on the live DB, a repeat of the July incident) because there is no target she can use: `seed-e2e.mjs` defaults to local `127.0.0.1`, and no Vercel-preview-for-Rodica exists. **Recommended default:** a stable "staging" Vercel deployment of `apps/web` wired to a DEDICATED non-prod Supabase project (EU), seeded synthetic, with Rodica-specific credentials; its Supabase ref MUST NOT be `dfotoodqvmjhbdcxyaxf` (prod). YELLOW authored the pt-PT instruction sheet template (`docs/ops/rodica-ambiente-de-teste.md`); the owner provisions the environment (infra, not a mergeable artifact) and fills the sheet. Until this exists, the purge (INC-02 half b) will recur. Note: a dedicated Supabase project is arguably a new resource - owner-confirmable.

### Q-PL-02-1 - Patient Marcações "cannot edit": time/estado/cancel already work - does Rodica need a FULL edit (service/therapist)? - ANSWERED 2026-07-28 (owner: NO full edit needed) (PL-02, CLOSED)
