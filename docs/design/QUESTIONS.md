...
- [ ] **AI recording — scoped-audio-signer env vars (W4-08).** The presigned S3 signer reads these from Vercel env (values from the vault-delivered scoped key — PutObject+GetObject on `osteojp-audio-intake` only, eu-central-1): **`AUDIO_S3_REGION`**, **`AUDIO_S3_BUCKET`**, **`AUDIO_S3_ACCESS_KEY_ID`**, **`AUDIO_S3_SECRET_ACCESS_KEY`**. Owner sets them in the Vercel project env (NEVER committed). Until set, the recorder's upload shows the "config" state. **Real-bucket round-trip (presigned GET 200) is a DEPLOY verification** — the signer + client PUT are mock-verified in CI (no AWS key locally, by design); the real 200 against `osteojp-audio-intake` is confirmed on the Vercel preview once the env is set. New (W4-08 2026-07-06). 
- [ ] **AI recording — M1 webhook env vars (W4-09).** The post-upload fire reads: **`M1_WEBHOOK_URL`** (André's Make.com scenario endpoint) and **`M1_WEBHOOK_API_KEY`** (value = vault key `osteojp-m1-webhook-key`, sent as the `x-make-apikey` header). Owner sets them in Vercel env (NEVER committed). Until set, the recorder shows "saved, processing pending" after upload (the audio is safely stored; the fire is deferred). **The real fire (200 from André's endpoint) is a DEPLOY/coordination check** — the payload contract, header-from-env, and non-2xx handling are mock-verified in CI. New (W4-09 2026-07-06).
