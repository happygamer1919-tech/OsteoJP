...
| W4-08 | presigned-put-flow (`docs/loops/wave-04/W4-08-presigned-put-flow.md`) | DONE (#) | backend+client (migration-free) | Depends W4-07. Backend signs presigned PUT with the scoped AWS key (`PutObject`+`GetObject` on `osteojp-audio-intake` only, eu-central-1) from Vercel env; browser PUTs direct to S3, never via a Vercel route (4.5 MB limit). CORS is André's side (locked to the EMR origins). Round-trip proof: signer response + presigned GET 200, credential-free. Key never printed/in code |
