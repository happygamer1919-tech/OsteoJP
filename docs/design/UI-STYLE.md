...
- **Brand equity (55 / 25 / 20, W6-06, made perceptible in W7-03):** structure is 55 percent white + grey (surfaces, borders, `text-*-primary`/`-secondary`); cyan (accent-2) is 25 percent for PRIMARY interaction (links/CTAs use `accent-2-700 #2F7E72` for text, AA 4.83:1; never the base `#45B9A7` for text); purple (accent-1 `#8B1863` + its tints) is 20 percent for SECONDARY emphasis. **W7-03 fixed the fact that purple was not actually visible:** it now has five DEFINED roles and is applied to all of them. (1) **Active tab**: both the 2px underline AND the label are `accent-1-700` (W6-06b purpled only the hairline under a near-black label, which read as no purple at all). (2) **Empty-state icon badge**: `text-accent-1-700` on `bg-accent-1-50` (8.43:1). (3) **Section headers** on the patient tabs: a 2px `border-accent-1-700` left rule. (4) **Row icon badges** (Documentos): `accent-1-700` on `accent-1-50`. (5) **Estatisticas chart**: the PEAK bar fills `accent-1-700`, the rest stay cyan, and every value is still printed as text so colour is never the only cue. **Purple is emphasis, not interaction:** the v2-green `Button` variants and the sidebar's active nav item are deliberately NOT repainted (that would re-brand the product's primary interaction colour and invalidate their AA analyses). Semantic status tints and the Agenda service tints are unchanged. Every purple pairing meets AA (tables in `docs/design/W6-06-color-equity-palette-plan.md` and `docs/loops/wave-07/W7-03-audit.md`). The bodychart `marker-*` palette is out of scope and never touched. Canonical hexes never drift (`packages/ui/src/tokens.test.ts`).
- **Empty states (W7-03):** exactly icon badge, title, subtitle, optional action. **Nothing renders above the icon.** The decorative azulejo motif band (`HeritageBand`) was removed platform-wide, along with the `EmptyState heritage` prop, so the ornament cannot be reintroduced by a call site. `HeritageDivider`, `HeritageCorners` and `HeritageFrame` are unaffected: they serve the shell and auth surfaces, not empty states.
- **Destructive actions (W7-03):** the "Ações destrutivas" block is a `<details>` disclosure, **collapsed by default**, bordered and titled with the error token. It is contained, not relocated: it still mounts where it always did. Never expand it by default, and never move it to another tab or menu without an owner ruling (Q-W7-03-1).
