...
- **Confirmed go-live sequence AFTER this loop (André-confirmed 2026-07-06) — orientation, NOT this loop's DoD:** once the **EMR half is merged**, the ordered path to production is: (1) **Ivan confirms to André** the EMR side is ready; (2) **joint dry run against a real uploaded object** (this loop's fire is the first such); (3) **canary run with the Jabra Speak 510 in the room**; (4) **pilot date set with Rodica**. **The first genuine fire carrying real audio + the `x-make-apikey` header + `audio_filename` is what confirms the full chain on André's side.** This loop delivers step (2)'s machine half (synthetic patient, draft lands `pending_review`); the canary/pilot (3–4) are follow-ups, not gated here. The **Jabra re-test is a follow-up**, not a blocker for this loop (owner ruling 2026-07-06).
