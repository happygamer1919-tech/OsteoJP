...
- **Bucket hardening (André-confirmed 2026-07-06, do NOT change):** **Block All Public Access ON** (access only ever via presigned URL); **encryption SSE-S3 — NEVER KMS** (KMS breaks presigned URLs); a **7-day lifecycle rule auto-deletes every object** — so **any presigned expiry set here must stay well under 7 days** (this loop's PUT/round-trip GET expiry is short; W4-09's fired GET is 1h).
- **Scoped IAM key (one dedicated signer):** the backend signs with the **vault-delivered scoped AWS key** limited to **`s3:PutObject` + `s3:GetObject` on `osteojp-audio-intake` only — no other bucket, no other action, NO list, NO delete**. Secret lives in **vault, referenced in Vercel env vars only — NEVER printed, NEVER in code, NEVER committed** (CLAUDE.md environment/secrets rule). The OsteoJP backend is the **ONLY** signer for both PUT and GET (same key).
...
- **CORS (RESOLVED, André-confirmed 2026-07-06):** André's bucket CORS is locked to **exactly three** browser-PUT origins — **`https://osteojp-platform.vercel.app`, `https://app.osteojp.pt`, `http://localhost:3000`**. **`osteojp-api.vercel.app` is deliberately EXCLUDED** (no browser PUTs originate there). The staff capture page must serve from one of the three; if it ever serves from another origin, **André must add it first**. CORS config is André's side (his AWS), not ours to set; a CORS failure from an unlisted origin is an André coordination item, not a code fix here.
- **Signing symmetry:** the backend signs BOTH PUT (this loop) and GET (W4-09) with the **same scoped key**. This loop signs the **PUT** and proves a subsequent **presigned GET** can read the object back (round-trip proof).
...
- Direct-to-S3 upload **fails on CORS** — CORS is confirmed for the three origins (`osteojp-platform.vercel.app`, `app.osteojp.pt`, `localhost:3000`); a failure means the capture page is serving from an **origin not on that list** (e.g. `osteojp-api.vercel.app`, deliberately excluded). Surface it as an André coordination item (he must add the new origin); do NOT work around it by proxying through Vercel.
