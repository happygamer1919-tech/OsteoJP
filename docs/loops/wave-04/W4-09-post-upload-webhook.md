...
- **Presigned GET:** signed by the OsteoJP backend with the scoped key (`s3:GetObject` on `osteojp-audio-intake`), **1h expiry**. **Bucket hardening (André-confirmed 2026-07-06):** SSE-S3 (never KMS), Block All Public Access ON, and a **7-day lifecycle auto-delete** — the **1h GET stays well under 7 days** with wide margin; do NOT lengthen it toward the lifecycle window.
- **M1 webhook contract fields (ALL mandatory on every fire, André-confirmed 2026-07-06):** `audio_url` (the presigned GET, 1h), `audio_filename` (e.g. `consultation.webm`), `patient_id`, `doctor_id`, `consultation_started_at`, `consultation_ended_at`, and the template `osteopathy`.
- **API-key header:** **`x-make-apikey`** (lowercase) on **every** fire, **value from vault key `osteojp-m1-webhook-key`** (referenced via env). **Missing or wrong header → `401` and the audio never enters André's scenario** (André-confirmed 2026-07-06). The key lives in **vault/env ONLY — never in chat, never in code, never committed**.
- **`audio_filename` matters downstream:** **André's transcription module reads the filename from THIS field — it is no longer hardcoded** (André-confirmed 2026-07-06); it is also the token André maps in Make (module 26 remap). It must be present and correct in the fire.
- **Timestamps come from W4-07** machine-stamps (they feed the idempotency key); this loop forwards them, does not re-derive them by hand. **Idempotency key (André-confirmed, unchanged):** `patient_id` + `consultation_started_at` + `consultation_ended_at` — forwarding the machine-stamped timestamps verbatim is what keeps a re-fire idempotent.
...
4. **Fire the M1 webhook:** POST the full contract (ALL fields mandatory) — `audio_url` (the 1h GET), `audio_filename`, `patient_id`, `doctor_id`, `consultation_started_at`, `consultation_ended_at`, template `osteopathy` — with header **`x-make-apikey`** read from **vault key `osteojp-m1-webhook-key`** (via env). A fire with a missing/wrong header is **401-rejected by André's side** (audio never enters the scenario), so the header is non-optional. Handle non-2xx (retry/log per the SPEC; surface a pt-PT error if user-facing).
5. **Tests:**
