...
**Board:** `docs/board/portal-board.json` @ `as_of 2026-08-12T02:30:00Z`.
**Scope:** every card not `status: shipped`. **59 cards.**
**Shipped and out of scope here:** 47. **Board total: 106.**

Generated by `docs/board/gen-triage.mjs`, not typed. It refuses to emit this file
unless every unshipped card lands in exactly one bucket, so the four counts below
always sum to the unshipped total and no card can be quietly dropped.

**Do not hand-edit this file — edit the generator.** The generator lived outside
the repo until 2026-08-11 and the cost showed up within hours: PR #867 corrected
the next-card label here by hand, and the next regeneration silently reverted it.
