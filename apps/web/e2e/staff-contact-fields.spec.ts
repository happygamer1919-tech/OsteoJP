 ...
 * (W8-02), re-pointed to the W12-40 card grid + Gerir modal.
 *
 * Proves the two profile fields end to end: an admin edits a DISPOSABLE staff
 * member's phone + professional job title in the Gerir modal's Contacto section,
 * and both persist across a full page reload — the job title surfaces on the card
 * (beside the role), and both prefill back into the edit form on reopen.
 *
 * Also pins the decoupling: a job title set on a member does NOT change the
 * permission role shown on that card (job_title is a display field, never the
 * auth role).
 *
...
test("Equipa: edit a disposable member's phone + job title; both persist across reload (W8-02)", async ({
  ...
  // 2. Find the new member's card and open its Gerir modal (Contacto is default).
  await page.goto("/admin/staff");
  const card = page.locator('[data-testid="equipa-card"]').filter({ hasText: fullName });
  await expect(card).toHaveCount(1);
  await card.getByRole("button", { name: "Gerir" }).click();

  const dialog = page.getByRole("dialog", { name: /Gerir/i });
  await expect(dialog).toBeVisible();

  // 3. The Contacto edit form starts EMPTY for phone + job title (ships null).
  ...
  // 5. Full reload: the card shows the job title beside the role, and the role
  //    itself is UNCHANGED (job title is decoupled from the permission role).
  await page.goto("/admin/staff");
  const savedCard = page.locator('[data-testid="equipa-card"]').filter({ hasText: fullName });
  await expect(savedCard).toContainText(jobTitle);
  await expect(savedCard).toContainText("Terapeuta"); // permission role, unchanged

  // 6. Reopen the modal: both fields are prefilled from the persisted values.
  await savedCard.getByRole("button", { name: "Gerir" }).click();
  const reopened = page.getByRole("dialog", { name: /Gerir/i });
